...
void SetUpCudaDevices()
{
	...
	int num_multiprocessors = prop.multiProcessorCount;

	BlockSize.x = 2 * num_multiprocessors;
...
{
	int binIdx = int(randomNumbers[id] / (MAX_RANDOM_NUMBER / NUMBER_OF_BINS));
	atomicAdd(&s[binIdx], 1);
}
__syncthreads();
