...
void addVectorsCPU(float *a, float *b, float *c, int n)
{
	...
	{ 
		c[id] = sqrt(cos(a[id])*cos(a[id])  + sin(a[id])*sin(a[id]) - 1.0 + a[id]*a[id]) + 
				sqrt(cos(b[id])*cos(b[id])  + sin(b[id])*sin(b[id]) - 1.0 + b[id]*b[id]);
	}
}
...
__global__ void addVectorsGPU(float *a, float *b, float *c, int n)
{
    ...
    {
        c[i] = sqrt(cos(a[i]) * cos(a[i])  + sin(a[i]) * sin(a[i]) - 1.0 + a[i] * a[i]) +
               sqrt(cos(b[i]) * cos(b[i])  + sin(b[i]) * sin(b[i]) - 1.0 + b[i] * b[i]);
    }
}
...
int main()
{
	...
	cudaMemcpyAsync(A_CPU, A_GPU, N*sizeof(float), cudaMemcpyDeviceToHost);
	cudaMemcpyAsync(B_CPU, B_GPU, N*sizeof(float), cudaMemcpyDeviceToHost);
