...
{	
	// Host "CPU" memory.				
	A_CPU = (float*)malloc(N*sizeof(float));//reserving memory on the CPU for a pointer that points 
	B_CPU = (float*)malloc(N*sizeof(float));// to an array of floats with N elements
	C_CPU = (float*)malloc(N*sizeof(float));
	
	// Device "GPU" Memory
	cudaMalloc(&A_GPU,N*sizeof(float));// reserving memory on the GPU for a pointer that points 
	cudaMalloc(&B_GPU,N*sizeof(float));// to an array of floats with N elements
...
{
	int id = threadIdx.x;//Cast the thread id values into the index of the the arrays
	int stride = blockDim.x;//The number of threads in each block, 
	
	for (int i = id; i<n; i += stride)// once its done adding an array index on each available thread the loop iterates 
	{//and keeps incrementing the array index by adding the number of threads we backtracked
	c[i] = a[i] + b[i];
	}
}
