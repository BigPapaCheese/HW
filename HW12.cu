// Name: Ben Williams
// Ray tracing
// nvcc HW12.cu -o temp -lglut -lGL -lGLU
/*
 What to do:
...
__constant__ sphereStruct s[NUMSPHERES];
// Globals variables
...
void display()
{
	// I added the if statement because GLUT was automatically redrawing the bitmap, resulting in two different compile times
	if(!alreadydone)
	{
	makeBitMap();
	alreadydone = true;
...
__global__ void makeSphersBitMap(float *pixels)
{
	...
	{
		hitValue = hit(pixelx, pixely, &dimingValue, s[i]);
...
{	
	...
	cudaMemcpyToSymbol(s, SpheresCPU, NUMSPHERES * sizeof(sphereStruct));
    free(SpheresCPU);
