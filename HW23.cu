...
dim3 GridSize1; //This variable will hold the Dimensions of your grid
dim3 GridSize;
...
void setUpDevices()
{
	...
	GridSize1.x = (n2 - 1)/BlockSize.x + 1; // This gives us the correct number of blocks.
...
{	
	...
	// Device "GPU" Memory
    cudaSetDevice(0);
	cudaMalloc(&A_GPU,n1*sizeof(float));
	cudaErrorCheck(__FILE__, __LINE__);
	cudaMalloc(&B_GPU,n1*sizeof(float));
	cudaErrorCheck(__FILE__, __LINE__);
	cudaMalloc(&C_GPU,n1*sizeof(float));
	cudaErrorCheck(__FILE__, __LINE__);
    cudaSetDevice(1);
...
int main()
{
    ...
    if(count < 2)
    {
        printf("Your GPU is lonely, get it some company...");
       exit(0); 
    }
    if(N%2==0)
    {
    ...
    cudaSetDevice(0);
	cudaMemcpyAsync(A_GPU, A_CPU, n1*sizeof(float), cudaMemcpyHostToDevice);
	cudaErrorCheck(__FILE__, __LINE__);
	cudaMemcpyAsync(B_GPU, B_CPU, n1*sizeof(float), cudaMemcpyHostToDevice);
	cudaErrorCheck(__FILE__, __LINE__);
    cudaSetDevice(1);
    ...
    cudaSetDevice(0);
	cudaMemcpyAsync(C_CPU, C_GPU, n1*sizeof(float), cudaMemcpyDeviceToHost);
	cudaErrorCheck(__FILE__, __LINE__);
	cudaSetDevice(1);
