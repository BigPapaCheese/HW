...
{	
	...
	int rem = BLOCK_SIZE - (N%BLOCK_SIZE);
	...
	cudaErrorCheck(__FILE__, __LINE__);
	cudaMalloc(&C_GPU,N*sizeof(float) + rem* sizeof(float));
	cudaMemset(C_GPU, 0, (N * sizeof(float)) + rem* sizeof(float));
	cudaErrorCheck(__FILE__, __LINE__);
}
...
__global__ void dotProductGPU(float *a, float *b, float *c, int n)
{
	...
	if (threadIndex == 0)
	{
		atomicAdd(&c[0], c_sh[threadIndex]);
	}
	__syncthreads();
}
...
int main()
{
	...
	for (int i = 0; i < count; i++) {
		...
		if (prop.maxThreadsPerBlock >= BLOCK_SIZE && (prop.maxGridSize[0]>= ((N-1)/BLOCK_SIZE) +1) && prop.major >= 3)
		{
			if (prop.major > max)
			{
				max = prop.major;
			...
			else if (prop.major == max)
			{
				if (prop.minor > maxmin)
				{
					gpuid = i;
				}
	...
	if(max == 0 )
	{
		printf("You don't have a gpu with a high enough max of blocks or threads per block, or you don't have a high enough compute capability");
		exit(0);
	}
	cudaSetDevice(gpuid);
	timeval start, end;
