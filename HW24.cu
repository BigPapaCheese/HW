...
void setup()
{
	...
	GridSize.x = (N0 - 1)/BlockSize.x + 1; //Makes enough blocks to deal with the whole vector.
	GridSize.y = 1;
	GridSize.z = 1;
...
__global__ void getForces(float3 *p, float3 *v, float3 *f, float *m, float g, float h, int n, int x, int N)
{
	float dx, dy, dz,d,d2;
	float force_mag;
...
__global__ void moveBodies(float3 *p, float3 *v, float3 *f, float *m, float damp, float dt, float t, int n, int x)
{	
	int i = threadIdx.x + blockDim.x*blockIdx.x;
...
void nBody()
{
	...
	{
        cudaSetDevice(0);
		getForces<<<GridSize,BlockSize>>>(PGPU, VGPU, FGPU, MGPU, G, H, N0, 0, N);
		cudaErrorCheck(__FILE__, __LINE__);
		moveBodies<<<GridSize,BlockSize>>>(PGPU, VGPU, FGPU, MGPU, Damp, dt, t, N0, 0);
		cudaErrorCheck(__FILE__, __LINE__);

        cudaSetDevice(1);
        getForces<<<GridSize1,BlockSize>>>(PGPU1, VGPU1, FGPU1, MGPU1, G, H, N1, N0, N);  
		cudaErrorCheck(__FILE__, __LINE__);
        ...
        // Overwrite first half of PGPU1 with first half of PGPU (from device 0 to 1)
        cudaMemcpyPeer(PGPU1, 1, PGPU, 0, N0 * sizeof(float3));
        cudaErrorCheck(__FILE__, __LINE__);
        
        // Overwrite second half of PGPU (starting at N0) with PGPU1's data (from device 1 to 0)
        cudaMemcpyPeer(PGPU + N0, 0, PGPU1 + N0, 1, N1 * sizeof(float3));
        cudaErrorCheck(__FILE__, __LINE__);
        cudaSetDevice(0); 
        cudaDeviceSynchronize();
